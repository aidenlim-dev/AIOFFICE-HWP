#!/usr/bin/env pwsh
# Local Codex install: junction plugins/aioffice-hwp into ~/plugins/aioffice-hwp
# and register it in ~/.agents/plugins/marketplace.json. The repo stays the
# source of truth, so local edits are live after a Codex restart.
# Uninstall with scripts/codex-uninstall-local.ps1.
[CmdletBinding()]
param(
  [switch] $Force
)
