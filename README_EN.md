...
- **convert** — `.hwp ↔ .hwpx` both directions via rhwp WASM. Not pixel-faithful — table structure and cell content survive, but cell shading, spacing, and page breaks can shift, and images do not render after conversion (verified; complex shapes untested)
...
- **Tables are preserved when editing `.hwp` or `.hwpx`.** Conversion (`.hwp ↔ .hwpx`) preserves table structure and cell content but not full visual fidelity (cell shading, spacing, and page breaks can shift) — prefer staying in the input's original format. Editing existing files keeps tables intact in either format: `.hwp` via byte-level raw-patch (cell content·text replace·paragraph/table insert·page setup·formatting), `.hwpx` via direct XML edit (cells·alignment·background·borders·rows/columns·insert_table·headers/footers·lists·footnotes·hyperlinks). Tables are also supported when creating new documents in either `.hwp` or `.hwpx`.
- **`.hwp ↔ .hwpx` conversion is not pixel-faithful.** Table structure and cell content survive and render in Hancom Docs, but cell background shading, spacing, and page breaks can shift, and **images do not render after conversion** (verified — the picture bytes survive but Hancom Docs doesn't draw them; complex shapes untested) — **edit in the input's original format** when fidelity matters (`.hwp` stays `.hwp`, `.hwpx` stays `.hwpx`). Only run `convert.js` when the user explicitly requests a format change.
