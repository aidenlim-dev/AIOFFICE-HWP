...
const HANDLERS = {
  setup_document(doc, op, cursor) {
    ...
    const orientationLabel = op.orientation
      ? (String(op.orientation).toLowerCase() === "landscape" ? "landscape" : "portrait")
      : (pd.landscape ? "landscape" : "portrait");
    log.push(`setup_document: ${op.page_size || "default"} ${orientationLabel}, margin=${op.margin_mm ?? "?"}mm, base_font=${op.base_font || "default"}`);
  },
