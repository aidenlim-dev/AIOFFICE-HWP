...
| `type` | Args | Notes |
|--------|------|-------|
| `apply_text_style` | `target`, + any of `color` (hex "FF0000"), `bold`, `italic`, `underline`, `size` (HWP units, 1000≈10pt), `highlight` (true → yellow / hex like "FFFF00" / false → strip all markers in doc), `strikethrough` (bool) | Two independent paths inside one op: `highlight` is an inline `<hp:markpenBegin color>...<hp:markpenEnd/>` marker pair spliced around `target` inside its `<hp:t>` node (charPr untouched). Everything else clones the run's current charPr (not `charPr[0]` — keeps font/size/borderFill consistent with surrounding body), mutates the cloned copy, and retargets **only the first run** whose text contains `target`. `retargeted: 1` = styled that one run. **Restyling every occurrence is not supported** — to restyle a specific later occurrence, use a longer unique substring as `target`. Bumps `hh:charProperties@itemCnt`. |
