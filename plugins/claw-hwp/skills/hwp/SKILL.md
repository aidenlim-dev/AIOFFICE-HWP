...
> - **In-place `apply_paragraph_style` on big `.hwp` (50+ pages, ktx-style)** → Hancom Docs ✓ (Phase B raw-patch ParaShape + BorderFill, verified). Dispatcher routes `apply_paragraph_style` through `cell-patch.js applyParagraphStyleInPlace` — clones the target paragraph's PARA_SHAPE, overlays style props (alignment / indent / margins / line spacing / spacing-before-after), appends to DocInfo + bumps `HWPTAG_ID_MAPPINGS` PARA_SHAPE count, rewrites PARA_HEADER `paraShapeId`. For `background_color`, also appends a solid-fill HWPTAG_BORDER_FILL and sets `ParaShape.border_fill_id` (1-based ID, mirroring Hancom Office's REF convention). No auto char shade — Hancom Office desktop doesn't apply it either; paragraph fill covers between-glyph gaps uniformly via the preserved PARA_LINE_SEG layout cache.
...
- **Big-form (50+ page) in-place `apply_text_style` and `apply_paragraph_style`** are both supported via raw-patch (CharShape, and ParaShape + BorderFill respectively) — Hancom Docs compatible (verified 2026-05-27).
