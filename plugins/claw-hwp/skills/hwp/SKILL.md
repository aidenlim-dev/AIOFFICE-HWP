...
> ⚠️ **`append_image` rules of thumb (Hancom Docs compatibility):**
>
...
> - **`append_image` on an existing `.hwp` (in-place add to a file that already exists)** → Hancom Docs ✗. The raw-patch implementation lands the three required pieces (new `BinData/BIN000N.<ext>` stream, DocInfo `HWPTAG_BIN_DATA` registration, body `gso` + `SHAPE_COMPONENT_PICTURE` referencing the new `bin_data_id`), but Hancom Docs still rejects the result. Falling back to rhwp's `exportHwp()` path doesn't help either: round-tripping any non-trivial existing file through rhwp's serializer produces output Hancom Docs rejects. This is an intrinsic limitation of the HWP serialization path, not something the skill can route around.
> - **Workarounds for the in-place case:** (a) build the entire document from scratch in a single payload so the file never round-trips, (b) pre-design the template with an image placeholder and use `replace_text` / `set_cell_text` to fill surrounding fields (those go through raw-patch and stay Hancom-compatible), or (c) open the rhwp-produced file in Hancom Office desktop, which accepts the round-trip even when Hancom Docs (the web viewer) doesn't.
