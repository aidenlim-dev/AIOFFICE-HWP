...
> - **`font_family` caveat**: rhwp resolves font names through the document's registered font table; arbitrary names ("맑은 고딕", "함초롬돋움", etc.) are silently accepted by `applyCharFormat` but fall back to the default font (`함초롬바탕`) on readback — the new name doesn't register itself. To change the visible font, pre-register it in your base template or open in Hancom Office and pick from the font dropdown. The op argument is preserved so future Phase B / a font-registration helper can light it up without API churn.
